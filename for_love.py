...
class MessageBox(QtWidgets.QWidget):
    ...
    def __init__(self, parent = None):
        ...
        self.setWindowTitle(u'love you forever')  # 仅仅设置窗体标题，不设置位置。
        self.setWindowIcon(QIcon('/home/john-conner/Pictures/grandma.jpeg'))
        # 调用QIcon构造函数时，我们需要提供要显示的图标的路径(相对或绝对路径)。同时注意：使用QIcon类型必须导入此模块from PyQt5.QtGui import QIcon
        self.setToolTip(u'愿时光不老')  # 调用setToolTip()方法,该方法接受富文本格式的参数,css之类。
        ...
        self.buttonOK = QtWidgets.QPushButton(u'爱你', self)
        ...
        self.buttonE = QtWidgets.QPushButton(u'不爱', self)
        # 因为需要增加按钮，所以我们引入了QPushButton类,该按钮是QPushButton类的一个实例。构造函数的第一个参数是按钮的标签。第二个参数是父窗口小部件。父窗口小部件是示例窗口小部件，它是通过QWidget继承的
        ...
        self.buttonNO = QtWidgets.QPushButton(u'滚', self)
        # 因为需要增加按钮，所以我们引入了QPushButton类,该按钮是QPushButton类的一个实例。构造函数的第一个参数是按钮的标签。第二个参数是父窗口小部件。父窗口小部件是示例窗口小部件，它是通过QWidget继承的
    ...
    def showDialogOK(self):
        QtWidgets.QMessageBox.information(self, "欧耶", "我也爱你,么么么么么么么哒～～～要一直在一起哦", QtWidgets.QMessageBox.Ok)
        self.CloseAllowed = 1

    def showDialogEE(self):
        '''
        QtWidgets.QMessageBox.information(self, "别纠结了", "你完了，", QtWidgets.QMessageBox.Ok)
        ...
        '''
        QtWidgets.QMessageBox.information(self, "必须爱", "我们要永远在一起", QtWidgets.QMessageBox.Ok)
        QtWidgets.QMessageBox.information(self, "必须爱", "我们要永远不分离", QtWidgets.QMessageBox.Ok)
        QtWidgets.QMessageBox.information(self, "必须爱", "我们要永远走下去", QtWidgets.QMessageBox.Ok)
